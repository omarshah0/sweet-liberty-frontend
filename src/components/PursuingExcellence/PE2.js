...
const PursuingExcellence = () => {
  ...
  return (
    <section className="relative pb-16">
      ...
      <InView threshold={0.3} triggerOnce>
        {({ ref, inView }) => (
          <motion.h2
            ...
            className="font-bebas px-[20px] text-[60px] leading-[60px] lg:text-5xl mb-6 text-brandLight lg:leading-none lg:text-pursue xl:w-3/5 xl:ml-40"
          >
      ...
      <div className="mb-11 px-[20px] lg:flex xl:items-center">
        <InView threshold={0.3} triggerOnce>
          {({ ref, inView }) => (
            <motion.p
              ref={ref}
              ...
              animate={inView ? { opacity: 1, y: 0 } : { opacity: 0, y: 50 }}
              transition={{ duration: 0.7 }}
              className="font-bebas text-base leading-[36px] lg:text-2xl mb-6 text-brandLight lg:w-100 lg:mr-8 xl:mr-20"
            >
              Situated in the up-and-coming Collins Park area alongside the Bass
              Art Museum, Sweet Liberty offers guests world-class cocktails and
              creative American food. Conceived and operated by award-winning
              bartender John Lermayer and Dan Binkiewicz along with
              restaurateurS David Martinez and MICHELLE BERNSTEIN, Sweet Liberty
              brings an intelligent selection of food and drink from
      ...
      <div className="xl:ml-40 relative">
          ...
          {({ ref, inView }) => (
            ...
            >
              <StaticImage
                ...
                className="w-[355px]"
              />
              <div className="absolute right-0 bottom-[-110px] flip">
                <StaticImage
        ...
        <h2 className="font-bebas text-[60px] lg:text-7xl md:text-hero leading-hero text-brandLight relative bottom-6 md:bottom-14 ml-6 md:ml-2 lg:ml-10">
          <InView threshold={0.2} triggerOnce>
            {({ ref, inView }) => (
              <motion.span
                ref={ref}
                initial={{ x: -200 }}
                animate={inView ? { x: 20 } : { x: -200 }}
          ...
          <InView threshold={0.2} triggerOnce>
            {({ ref, inView }) => (
              <motion.span
                ref={ref}
                initial={{ x: 200 }}
                animate={inView ? { x: 80 } : { x: 200 }}
                transition={{ duration: 1, ease: "linear" }}
                className="inline-block"
              >
                <TitleStroke
                  className="relative xl:left-80 xl:ml-9 tracking-wide md:tracking-normal"
                  strokeWidth="1px"
                >
                  Excellence
                </TitleStroke>
